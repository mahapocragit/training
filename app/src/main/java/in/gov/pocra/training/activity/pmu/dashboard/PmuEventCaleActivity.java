...
import in.gov.pocra.training.activity.pmu.pmu_upcoming_event.AdaptorPmuComingEvent;
...
public class PmuEventCaleActivity extends AppCompatActivity implements OnDateSelectedListener, ApiCallbackCode, OnMultiRecyclerItemClickListener {
    //
    ...
    protected void onResume() {
        ...
        comingEventTypeId = getIntent().getStringExtra("comingEventType");
        if (comingEventTypeId.equalsIgnoreCase("1")){
           ...
           getPmuAllEventsDates();
            String cDate = ApUtil.getDateByTimeStamp(ApUtil.getCurrentTimeStamp());
            if (sDate.equalsIgnoreCase("")){
                getAllEventsAgainstDate(cDate);
            }else {
                getAllEventsAgainstDate(sDate);
            }
        }
    }
...
//         JSONArray dateArray = ApUtil.getDateBetweenTwoDate("05-04-2019","10-04-2019");
    ...
    private void getPmuEventsDates() {
        ...
        try {
            ...
            Call<JsonObject> responseCall = apiRequest.pmuEventDateListRequest(requestBody);

            DebugLog.getInstance().d("pmu_event_dates_param=" + responseCall.request().toString());
            DebugLog.getInstance().d("pmu_event_dates_param=" + AppUtility.getInstance().bodyToString(responseCall.request()));
    ...
    private void getPmuAllEventsDates() {
        try {
            JSONObject jsonObject = new JSONObject();
            ...
            Retrofit retrofit = api.getRetrofitInstance();
            APIRequest apiRequest = retrofit.create(APIRequest.class);
            Call<JsonObject> responseCall = apiRequest.pmuAllEventDateListRequest(requestBody);

            DebugLog.getInstance().d("pmu_all_event_dates_param=" + responseCall.request().toString());
            DebugLog.getInstance().d("pmu_all_event_dates_param=" + AppUtility.getInstance().bodyToString(responseCall.request()));
            api.postRequest(responseCall, this, 1);
    ...
    private void getEventsAgainstDate(String sDate) {
        ...
        try {
            ...
            Retrofit retrofit = api.getRetrofitInstance();
            APIRequest apiRequest = retrofit.create(APIRequest.class);
            Call<JsonObject> responseCall = apiRequest.pmuEventListByDateRequest(requestBody);
    ...
    private void getAllEventsAgainstDate(String sDate) {
        ...
        try {
            ...
            Retrofit retrofit = api.getRetrofitInstance();
            APIRequest apiRequest = retrofit.create(APIRequest.class);
            Call<JsonObject> responseCall = apiRequest.pmuAllEventListByDateRequest(requestBody);
    ...
    public void onDateSelected(@NonNull MaterialCalendarView widget, @NonNull CalendarDay date, boolean selected) {
        ...
        sDate =  ApUtil.getFormatedDateByDate(selectedDate);    // Date format is dd-mm-yyyy
        if (comingEventTypeId.equalsIgnoreCase("1")){
            getEventsAgainstDate(sDate);
        }else {
            getAllEventsAgainstDate(sDate);
            ...
            if (jsonObject != null) {
                ...
                if (i == 2) {
                    ...
                    if (responseModel.isStatus()) {
                        ...
                        if (scheduleJSONArray.length()>0){
                            AdaptorPmuComingEvent adaptorPmuComingEvent = new AdaptorPmuComingEvent(this,scheduleJSONArray, "",true, this);
                            eCalenderRView.setAdapter(adaptorPmuComingEvent);

                            //setDateToCalenderFromresponse(scheduleJSONArray);

                        }else {
                            UIToastMessage.show(PmuEventCaleActivity.this,responseModel.getMsg());
                        }

                    } else {
                        AdaptorPmuComingEvent adaptorPmuComingEvent = new AdaptorPmuComingEvent(this,null, "",false,this);
                        eCalenderRView.setAdapter(adaptorPmuComingEvent);
                        UIToastMessage.show(this, responseModel.getMsg());
                    }
                }
                ...
                if (i == 3) {
                    ...
                    if (responseModel.isStatus()) {
                        JSONArray scheduleJSONArray = jsonObject.getJSONArray("data");
                        if (scheduleJSONArray.length()>0){
                            AdaptorPmuComingEvent adaptorPmuComingEvent = new AdaptorPmuComingEvent(this,scheduleJSONArray, "",false, this);
                            eCalenderRView.setAdapter(adaptorPmuComingEvent);
//                            setDateToCalender(scheduleJSONArray);
//                            setDateToCalenderFromresponse(scheduleJSONArray);
                        }else {
                            UIToastMessage.show(PmuEventCaleActivity.this,responseModel.getMsg());
                        }
